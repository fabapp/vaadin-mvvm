...
package de.appblocks.vaadin.cdi;
...
/**
 * Provider class for lookup of CDI BeanManager in Tomcat 7/8 
 * @see: http://dev.vaadin.com/ticket/13884
 */
public class TomcatCDIUIProvider extends DefaultUIProvider implements Serializable {
	...
	Class<? extends UI> rootUI() {
		Set<Bean<?>> rootBeans = AnnotationUtil.getRootUiBeans(getBeanManager());
	...
	private Bean<?> scanForBeans(Class<? extends UI> type) {
		...
		Set<Bean<?>> beans = getBeanManager().getBeans(type, new AnnotationLiteral<Any>() {
		});

		if (beans.isEmpty()) {
			getLogger().warning("Could not find UI bean for " + type.getCanonicalName());
			return null;
		}

		if (beans.size() > 1) {
			getLogger().warning("Found multiple UI beans for " + type.getCanonicalName());
			return null;
		}
